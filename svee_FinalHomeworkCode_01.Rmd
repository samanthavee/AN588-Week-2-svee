...
output: 
  html_document:
    ...
    theme: cosmo
---
# **3 ways that peer commentary helped**

1. Good feedback lets me know that I'm on the right track! I will continue making comments like this for future assignments. <br><br>
2. Re: Challenge 1 - I named my variables v1 and v2 to represent my first and second attempts. I can see how these would be more redundant if I was working on something bigger or more complex. The only thing I did was change the name of v1.1 to v1 in the last line of code for my first attempt's chunk, updating the variable to v1.1 seemed redundant. But moving forward, I will be more mindful of how I name my variables, especially when the code becomes more involved/complex. <br><br>
3. Re: Challenge 4 - I agree that printing the lists for Haplorhines and Strepsirrhines makes the output
in the knitted doc look a bit messy and cluttered. I followed Jimmy's suggestion and removed these lines of code and only printed the final `Primates` list.
...
v1[seq(from = 1, to = length(v1), by = 3)]
```
...
v2[seq(from = 1, to = length(v2), by = 3)]
```
...
```{r challenge3b}
# comes from row 1 and column 1 (1,1) of matrix (1,2)
a[1,1,1,2]

# extracting elements from row 2 and column 3 (2,3) of matrices (2, 1:4)
a[2,3,2,]
